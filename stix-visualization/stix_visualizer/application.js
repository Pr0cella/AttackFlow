...
require(["domReady!", "stix2viz/stix2viz/stix2viz"], function (document, stix2viz) {
    ...
    function graphViewClickHandler(event, edgeDataSet, stixIdToObject)
    {
        if (event.nodes.length > 0)
        {
            ...
            if (stixObject)
                window.AttackFlowStixViz?.onSelect?.(stixObject.get("id"));
            if (stixObject)
    ...
    function listViewClickHandler(event, edgeDataSet, stixIdToObject)
    {
        ...
        if (clickedItem.tagName === "LI")
        {
            ...
            window.AttackFlowStixViz?.onSelect?.(stixId);
    ...
    function applyThemeToGraph(graphView)
    {
        ...
        let textColor = rootStyle.getPropertyValue("--text-primary").trim();
        let edgeColor = rootStyle.getPropertyValue("--text-secondary").trim();
        if (!textColor) textColor = mode === "dark" ? "#e5e5e5" : "#0f172a";
        if (!edgeColor) edgeColor = mode === "dark" ? "#a3a3a3" : "#475569";

        graphView.graph.setOptions({
            nodes: {
                font: { color: textColor }
            },
            edges: {
                font: { color: edgeColor }
            }
        });
    }

    // Expose a minimal API for host apps (AttackFlow builder)
    window.AttackFlowStixViz = {
        onSelect: null,
        render: function (content, customConfig) {
            return vizStixWrapper(content, customConfig);
        },
        reset: function () {
            return resetPage();
        },
