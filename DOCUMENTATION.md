...
- Metadata normalization handles `cveEntries`, legacy `cveId`/`cveIds`, and legacy `cves` arrays in nested or flat assignment metadata. Array entries retain source order, with current entries before legacy entries when both keys are present. Same-ID rows remain separate so differing score or vector evidence is not discarded; exports use the current `cveEntries` representation.
...
Imported custom library entries enforce the same field limits the editor enforces: `CONFIG.display.maxNameLength` (200) for `name`, `maxCustomDescLength` (2000) for `description`, and `maxCustomLabels` / `maxLabelLength` (20 labels of 50) for `labels`. A longer imported name is truncated to the limit rather than rejected, so names an analyst can create and edit also survive export and reimport.
Native kill-chain validation requires both the parsed document root and its `assignments` container to be non-null, non-array objects. Invalid shapes are rejected before assignments, groups, layout, document fields, or the custom STIX library are mutated. Empty assignment objects and documents containing only some recognized phases remain supported.
