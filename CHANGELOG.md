...
- **Composer Structural Validation**: Composer identifiers and references, dictionary/hash/extension keys, kill-chain values, granular-marking selectors and timestamps are validated against the formats STIX 2.1 defines, with a few deliberate limits listed in DOCUMENTATION.md, instead of being silently rewritten
- **Composer STIX 2.1 Formats**: The STIX Composer now accepts every granular-marking selector STIX 2.1 allows, including `id` and paths through dictionary keys with capitals or hyphens such as `external_references.[0].hashes.SHA-256`; kill-chain names and phases written as free text, such as `Mitre Attack`; and leap-second timestamps (`23:59:60` on the last day of a month), which the visualizer timeline now orders correctly (its label shows them as `23:59:59.999`); and identifiers and references with uppercase hex digits, such as `indicator--F81D4FAE-7DEC-11D0-A765-00A0C91E6BF6`, which were refused or silently dropped before and are now stored and exported in lowercase, with the import message giving the number of values lowercased. Extension names that are extension-definition ids are lowercased too, so they keep matching their definition; two names that differ only in case are both kept as written, and the import message says so. The type name before `--` must still be lowercase. Selectors that start with a name shorter than three characters or with a capital letter, and keys shorter than three characters in `hashes` properties and external-reference hashes, are reported as invalid
- **Composer Kill Chain Editor**: Objects whose kill chain is named like a built-in JavaScript property, such as `constructor`, can be opened in the editor again, and a kill chain or phase missing from the configuration is shown as stored; before, the editor showed the first configured value instead and saved it over the stored one as soon as a kill-chain field was changed
- **Composer Object Types**: Objects of type `constructor` are no longer treated as a supported type; they are handled like any other unsupported type
